...
$(document).ready(function(e) {
	...
	$("#btn_save").click(function(){
		...
		$.post("modules/check_add_resolution.php",{values: values},function(data){
			if(data==""){
				window.location="index.php?page=resolutions";
			}else{
	...
	$(".btn_delete").click(function(data){
		...
		if (quest==true){
			$.post("modules/check_delete_resolution.php",{rec_id: $(this).attr("rec_id")},function(data){
				if(data==""){
					window.location="index.php?page=resolutions";
				}
			});	
	...
	$("#"+rec_id+" .option .save_edit").show();
		...
		$(".save_edit").click(function(){
			...
			$.post("modules/check_edit_resolution.php",{rec_id: rec_id2, values: values},function(data){
				if(data==""){
					mofo();
				}
			});
	...
	<thead><th>ID</th><th>Resolutions Name</th><th>Description</th><th>Options</th></thead>
            <?php
			...
			?>
            		<tr id="add_record"><td width="30px"><input type="text" size="3" watermark="ID" class="input"></td>
                    	<td><input type="text" watermark="Resolution Name" size="30" class="input"></td>
                        <td><input type="text" watermark="Description" size="50" class="input"></td>
